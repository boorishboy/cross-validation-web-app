...
for i in [NNLS, OLS]:
    outliers[i] = [(bench, predicted, actual, 100 * (predicted - actual) / actual) if abs(predicted - actual) /
    ...
    stddev_abs_percentage_error[i] = np.sqrt(mean_squared_error(np.full(
        y.shape, mean_abs_percentage_error[i]), predicted[i] / y - np.full(y.shape, 1.0)))
    print("STDDEV(MAPE_%s) = %.5f%%" %
          (get_mode_name(i), stddev_abs_percentage_error[i] * 100.0))

    stddev_relative_error[i] = np.sqrt(mean_squared_error(np.full(
        y.shape, mean_percentage_error[i]), predicted[i] / y - np.full(y.shape, 1.0)))
